...
- **Self-service** — sign in (local password or OIDC SSO via auth) and change password via Settings.
- **Machine tokens** — open to every signed-in user. Issue bearer tokens for automated callers (CI, scripts, service accounts). Each token is bound to the issuing user and authenticates with `Authorization: Bearer <token>`. Three orthogonal scope dimensions: **project** (token rejected on any other project), **env** (further restricted to one environment within the project), and **read-only** (blocks all writes). The portal scope picker is filtered to scopes the current user can actually use: `Unscoped` and `<project> — all environments` options appear only when the user has a project-level (env_id IS NULL) membership row; env-only members see only their specific envs. Server admins see every scope. The same rules are enforced server-side on submit. Default 90-day TTL, overridable via Go-duration `expires_in`.
- **Cert principals** — open to every signed-in user. Map a client certificate's SPIFFE URI SAN or email SAN to vault authorization scope; callers presenting a matching cert authenticate via the mTLS handshake without a bearer token. Each principal carries the same scoping dimensions as a machine token (project required, env optional, read-only flag) plus an expiry independent of the cert lifetime. The portal scope picker is filtered by the user's project memberships and same access rules are enforced server-side. Identifier must be exactly one of a `spiffe://…` URI or a syntactically valid email address. Per-user list semantics — each user manages the principals they registered.
- **Projects** — open to every signed-in user. Server admins see all projects; non-admins see only the projects they are a member of. Project-level actions are gated by membership role (most-specific env row wins, matching the JSON API): viewer can browse and reveal; editor can create/edit/import/export/delete secrets and manage environments; owner can rotate the envelope key, delete the project, and manage members. Project creation itself is server-admin only — the creating admin is auto-added as project owner. Action buttons (Add env, Rotate key, Delete project, member management) are hidden in the UI when the viewer's effective project role is below the required level.
- **Secrets** — per-environment secret browser: create a new secret, edit (set a new version), list keys + version count + last-modified, reveal current value, view version history, reveal any historical version, roll back, delete. Bulk import via `.env` upload and download of a project+env's secrets as a `.env` file. Every set / reveal / export key is audit-logged with a masked preview; reveal and create/edit submissions POST (never GET) so values cannot leak through URLs, history, or referrer headers. **Rollback is forward-only**: rolling back to v3 creates a new v(max+1) whose content is a copy of v3's, then sets that as current. Version numbers never decrease, so version history is monotonic and easy to audit (`secret.set` / `secret.rollback` / `secret.delete`).
- **Admin → Users** — server-admin only. Create users, promote/demote between member and admin (last-admin demotion is rejected to keep vault administrable), deactivate (revokes all the user's tokens, matching SCIM deprovision semantics), reset password.
...
`VAULT_ADMIN_DATABASE_URL` is optional. When set, schema migrations run under this credential (owner/DDL role) at startup; the runtime connection then uses the least-privilege `VAULT_DATABASE_URL` credential. When absent, migrations are skipped — useful for normal restarts where the schema is already up to date.
...
| `VAULT_WORKLOAD_CA` | — | Single workload CA root used as the fallback for every per-channel CA env var (`VAULT_DB_CA`, `VAULT_ADMIN_DB_CA`, `VAULT_NATS_CA`, `VAULT_SCIM_MTLS_CA`, `VAULT_API_CLIENT_CA`). Set this alone in deployments that issue all internal certs from one CA; set per-channel vars to override individually. |
...
| `VAULT_API_CLIENT_CA` | falls back to `VAULT_WORKLOAD_CA` | Path to a PEM-encoded CA certificate. When set (or inheriting from the workload CA), the server requests a client certificate during the TLS handshake and verifies it against this CA. Used to enable SPIFFE/mTLS authentication (see [SPIFFE/mTLS Principals](#spiFFEmtls-principals)). Most API mTLS callers are workload services using the same workload CA as DB/NATS, so the default deployment pattern ("set `VAULT_WORKLOAD_CA`, leave per-channel CAs empty") gets API mTLS for free. Set `VAULT_API_CLIENT_CA` separately only when API client certs come from a different trust anchor (e.g. a corporate workforce PKI for human admin mTLS). |
...
All four base vars (`VAULT_OIDC_ISSUER`, `VAULT_OIDC_CLIENT_ID`, `VAULT_OIDC_CLIENT_SECRET`, `VAULT_OIDC_REDIRECT_URI`) must be set together to enable OIDC. Setting only some of them is a configuration error. See [`docs/oidc-sso-design.md`](docs/oidc-sso-design.md) for the full integration guide.
