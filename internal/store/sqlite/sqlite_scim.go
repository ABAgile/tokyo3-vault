...
func (s *DB) ListSCIMGroupRolesByExternalID(ctx context.Context, scimExternalID string) ([]*model.SCIMGroupRole, error) {
	rows, err := s.db.QueryContext(ctx,
		`SELECT id, scim_external_id, display_name, project_id, env_id, role, created_at FROM scim_group_roles WHERE scim_external_id = ? ORDER BY created_at`, scimExternalID,
	)
	if err != nil {
		return nil, err
	}
...
func (s *DB) GetSCIMGroupRole(ctx context.Context, id string) (*model.SCIMGroupRole, error) {
	r := &model.SCIMGroupRole{}
	err := s.db.QueryRowContext(ctx,
		`SELECT id, scim_external_id, display_name, project_id, env_id, role, created_at FROM scim_group_roles WHERE id = ?`, id,
