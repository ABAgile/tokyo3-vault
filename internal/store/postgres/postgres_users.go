...
func (s *DB) GetUserBySCIMExternalID(ctx context.Context, externalID string) (*model.User, error) {
	row := s.db.QueryRowContext(ctx,
		`SELECT `+userCols+` FROM users WHERE scim_external_id = $1`, externalID)
	u, err := scanUser(row)
	if err == sql.ErrNoRows {
		return nil, store.ErrNotFound
	}
...
func (s *DB) SetUserSCIMExternalID(ctx context.Context, userID, externalID string) error {
	...
	res, err := s.db.ExecContext(ctx,
		`UPDATE users SET scim_external_id = $1 WHERE id = $2`, arg, userID)
	if err != nil {
		return err
	}
...
func (s *DB) SetUserRole(ctx context.Context, userID, role string) error {
	res, err := s.db.ExecContext(ctx, `UPDATE users SET role = $1 WHERE id = $2`, role, userID)
	if err != nil {
		return err
	}
	n, _ := res.RowsAffected()
