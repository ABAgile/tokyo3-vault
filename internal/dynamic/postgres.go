...
func (p *PostgresIssuer) Issue(ctx context.Context, kp bcrypto.KeyProvider, backend *model.DynamicBackend, role *model.DynamicRole, ttl time.Duration) (username, password string, expiresAt time.Time, err error) {
...
func (p *PostgresIssuer) Revoke(ctx context.Context, kp bcrypto.KeyProvider, backend *model.DynamicBackend, revocationTmpl, username string) error {
	cfg, err := decryptConfig(ctx, kp, backend)
	if err != nil {
...
func decryptConfig(ctx context.Context, kp bcrypto.KeyProvider, backend *model.DynamicBackend) (pgConfig, error) {
	plaintext, err := bcrypto.DecryptEnvelope(ctx, kp, backend.EncryptedConfigDEK, backend.EncryptedConfig)
	if err != nil {
		return pgConfig{}, err
	}
	var cfg pgConfig
